({
	...
	keyPressControllerContact : function(component, event, helper) {
      ...
      // else close the lookup result List part.   
        if( getInputkeyWord.length > 0 ){
            ...
            helper.searchHelperContact(component,event,getInputkeyWord);
        }
        else{  
            component.set("v.listOfSearchRecordsContact", null ); 
  ...
  // automatically call when the component is done waiting for a response to a server request.  
    hideSpinnerContact : function (component, event, helper) {
        var spinnerCon = component.find('spinnerCon');
        var evt = spinnerCon.get("e.toggle");
        evt.setParams({ isVisible : false });
        evt.fire();    
    },
 // automatically call when the component is waiting for a response to a server request.
    showSpinnerContact : function (component, event, helper) {
        var spinnerCon = component.find('spinnerCon');
        var evt = spinnerCon.get("e.toggle");
